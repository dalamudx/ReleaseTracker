...
- **单一管理员，而非 RBAC**。`system.admin_user_id` 标识一个稳定管理员。所有业务资源和系统管理操作仅限该用户；现有非管理员记录只能使用自助认证端点。用户注册已禁用。
- **没有角色或租户**。稳定管理员 ID 并不是可配置的角色系统。修改管理员用户名不会转移权限；引用格式错误或指向不存在用户时，启动会安全失败。
- **引导管理员密码**。全新安装首次启动会生成随机的一次性密码，并仅在 INFO 日志中记录一次；现有安装凭证不会改变。删除引导管理员会导致后续启动失败，而不会生成新密码；请限制日志访问并首次登录后立即修改密码。
- **显式 OIDC 绑定**。OIDC 仅接受绑定到现有管理员的一个已验证 issuer + subject，绝不会创建用户。绑定与解绑都需要当前本地密码，本地密码必须保留为恢复路径。
...
- Webhook 遇到限流或传输失败时只会进行少量、有上限的重试；失败事件不会进入队列等待后续重放。
- 不支持带自定义 HTTP 头的 webhook。服务商凭证可能因此包含在已存储的 URL 中；应用日志不会记录完整 Webhook URL。
- Webhook 与 HTTP 健康检查目标必须全部解析为公网地址并使用允许的 HTTP(S) 端口；不支持私有/容器网络目标或重定向。
...
- 前端仅提供 zh / en 两种语言。
- OIDC 仅用于为已绑定管理员获取本地 JWT 会话；API 不直接接受 IdP 签发的令牌。
