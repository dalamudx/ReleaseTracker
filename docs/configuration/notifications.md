...
Notifier {
  name:        string
  type:        webhook      # 当前仅支持 webhook
  url:         string       # HTTP/HTTPS 目标地址
  events:      list<string> # 订阅的事件，空列表等价于禁用
  enabled:     bool
  language:    en | zh
...
- `URL`：Webhook 目标。必须是完整的 HTTP/HTTPS 地址。
- `events`：勾选订阅的事件。未勾选的事件不会发送。
- `language`：消息语言，`en` 或 `zh`。
- `enabled`：关闭后即使事件触发也不会发送。
