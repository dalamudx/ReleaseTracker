...
访问 <http://localhost:8000>（或反向代理的对外地址）。全新安装首次启动会自动创建 `admin` 管理员，并生成加密安全的随机一次性引导密码。请从启动日志中查看：

```bash
docker logs releasetracker 2>&1 | grep "one-time bootstrap admin password"
```

该密码仅在首次成功初始化时以 INFO 级别记录一次，且不会通过 API 返回。现有安装会保留当前管理员凭证；如果引导管理员之后被删除，ReleaseTracker 将拒绝启动而不会重新生成密码，请从可信备份恢复管理员或数据库。

!!! danger "请立即修改引导密码"
    使用日志中的密码登录后，第一时间修改密码：打开**左下角用户菜单 → 用户设置 → 修改密码**。请限制启动日志的访问权限。

## 4. 快速开始

服务安装并可访问后，可以按下面的工作流完成首次配置：

1. **打开 Web UI 并登录**：访问部署地址，使用启动日志中的一次性引导密码登录 `admin`；首次登录后先修改密码。
...
安装完成后，参阅[使用配置说明](../configuration/flow.md)了解追踪器、凭证与执行器的完整配置流程。