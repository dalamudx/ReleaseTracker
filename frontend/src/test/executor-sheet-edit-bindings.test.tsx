  ...
  it("shows existing Kubernetes workload target and service binding when editing", async () => {
    getExecutorConfigMock.mockResolvedValue(createExecutorConfig({
      ...
      target_ref: {
        ...
        kind: "Deployment",
        name: "sample_canary",
        services: [{ service: "sample_canary-lkdsjfh", image: "registry.example.test/example/sample-canary:0.7.0-rc21" }],
        service_count: 1,
      },
          ...
          sources: [
            {
              ...
              channel_type: "container",
              enabled: true,
              channel_config: { image: "registry.example.test/example/sample-canary" },
              release_channels: [
                { release_channel_key: "image-stable", name: "stable", type: "release", enabled: true },
              ],
              channel_rank: 0,
              source_key: "image",
              source_type: "container",
              source_config: { image: "registry.example.test/example/sample-canary" },
              source_rank: 0,
            },
          ],
        })]}
