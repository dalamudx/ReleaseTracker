...
export function NotifierSettings() {
    ...
    useEffect(() => {
        void Promise.resolve().then(loadNotifiers)
    }, [loadNotifiers])

    const handleDelete = async () => {
        if (pendingDeleteNotifierId === null) {
            return
        }
        try {
            await api.deleteNotifier(pendingDeleteNotifierId)
            setNotifiers(notifiers.filter(n => n.id !== pendingDeleteNotifierId))
            setTotal(total - 1)
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>{t('settings.notifications.dialog.deleteConfirm')}</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} value={field.value || "en"}>
                                        ...
                                        <SelectContent>
                                            <SelectItem value="en">{t('settings.notifications.dialog.languages.en')}</SelectItem>
                                            <SelectItem value="zh">{t('settings.notifications.dialog.languages.zh')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                    <FormDescription>
