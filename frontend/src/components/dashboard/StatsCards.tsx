                ...
                <Card key={item.label} className="overflow-hidden h-32 glass-card">
                    <CardContent className="p-0 flex h-full">
                        ...
                        <div className="w-[30%] min-w-[140px] flex flex-col justify-center items-center p-4 text-center border-r border-border/10">
                            {loading ? (
                                <div className="space-y-2 w-full flex flex-col items-center">
                                    ...
                                    <div className="h-3 w-20 bg-muted/50 rounded mt-2 animate-pulse"></div>
                                </div>
                            ) : (
                                <>
                                    <div>
                        ...
                        <div className="flex-1 min-w-0 flex flex-col">
                            <div className="flex-1 p-2 pb-0 min-h-0 relative">
                                {loading ? (
                                    <div className="absolute inset-2 bg-muted/10 rounded animate-pulse"></div>
                                ) : (
                                    item.data.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart
                                                ...
                                                layout="vertical"
                                                margin={{ left: item.showCategoryLabels ? 10 : 0, right: 40, top: 0, bottom: 0 }}
                                            >
                                                <XAxis type="number" hide />
                                                <YAxis
                                                    type="category"
                                                    dataKey="name"
                                                    hide={!item.showCategoryLabels}
                                                    width={item.showCategoryLabels ? 56 : 0}
                                                    axisLine={false}
                                                    tickLine={false}
                                                    interval={0}
                                                    tickMargin={6}
