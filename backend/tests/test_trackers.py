...
async def test_get_trackers_pagination(authed_client):
    # Create 2 trackers
    authed_client.post("/api/trackers", json={
        "name": "t1", "type": "github", "repo": "o/r1", "interval": 60
    })
    authed_client.post("/api/trackers", json={
        "name": "t2", "type": "github", "repo": "o/r2", "interval": 60
    })
...
async def test_update_tracker(authed_client):
    # Setup
    authed_client.post("/api/trackers", json={
        "name": "update-test", "type": "github", "repo": "o/r", "interval": 60
    })
    
    # Update interval
    response = authed_client.put("/api/trackers/update-test", json={
...
async def test_delete_tracker(authed_client):
    authed_client.post("/api/trackers", json={
        "name": "del-test", "type": "github", "repo": "o/r", "interval": 60
    })
