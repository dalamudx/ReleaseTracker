            ...
            if releases and tracker_config:
                ...
                for release in unique_releases:
                    ...
                    if result["is_new"]:
                        ...
                        logger.info(f"Republish detected: {tracker_name} -> {release.version} (commit: {old_commit_short} → {new_commit_short})")
    ...
    async def _create_tracker(self, config: TrackerConfig) -> BaseTracker:
        ...
        if config.credential_name:
            ...
            if credential:
                ...
                logger.warning(f"Credential '{config.credential_name}' referenced by tracker {config.name} not found, using anonymous access")
