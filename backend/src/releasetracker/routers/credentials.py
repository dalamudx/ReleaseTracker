...
async def get_credentials(
    ...
    for cred in credentials:
        ...
        if cred.token:
            # 显示前4位和后4位
            cred_dict["token"] = (
                f"{cred.token[:4]}...{cred.token[-4:]}" if len(cred.token) > 8 else "****"
            )
...
async def get_credential(
    ...
    if cred_dict.get("token"):
        cred_dict["token"] = (
            f"{credential.token[:4]}...{credential.token[-4:]}"
...
async def create_credential(
    credential_data: dict, storage: Annotated[SQLiteStorage, Depends(get_storage)]
):
    """创建新凭证"""
    try:
        # 检查名称是否重复
