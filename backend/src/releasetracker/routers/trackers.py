...
@router.get(
    "/{tracker_name}", response_model=TrackerStatus, dependencies=[Depends(get_current_user)]
)
async def get_tracker(tracker_name: str, storage: Annotated[SQLiteStorage, Depends(get_storage)]):
    ...
    if not status:
        ...
        return TrackerStatus(
            name=tracker_config.name, type=tracker_config.type, enabled=calculated_enabled
        )
