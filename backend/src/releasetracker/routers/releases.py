...
async def get_releases(
    ...
    total = await storage.get_total_count(
        tracker_name=tracker, search=search, prerelease=prerelease, include_history=include_history
    )
...
@router.get(
    "/releases/latest", response_model=list[Release], dependencies=[Depends(get_current_user)]
)
