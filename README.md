...
ReleaseTracker 是一款轻量级、可配置的版本追踪与更新编排工具。追踪 GitHub、GitLab、Gitea、Helm Chart 与 OCI 容器镜像仓库的 release / tag，并将版本变化关联到 Docker、Podman、Portainer、Kubernetes、Helm 等运行时目标。
...
常用：`make install`、`make dev`、`make lint`、`make build`、`make version VERSION=x.y.z`。完整列表运行 `make help`。
