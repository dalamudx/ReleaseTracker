...
更多界面截图和功能场景说明见：[功能截图与场景说明](FEATURES.md)。
...
- [ ] 执行器快照与恢复：已完成多快照历史存储、保留策略、调度器预更新快照，以及 Portainer Stack 快照 / 恢复适配；仍需完成快照列表 / 详情 API、手动回滚服务与 API、快照脱敏、前端快照面板和回滚操作。
- [ ] 版本更新后的健康检查：已完成 `health_check` 数据库字段、后端配置模型、校验规则与存储读写；仍需接入执行器调度流程，实现 runtime-native / Helm / HTTP / TCP 探针、Recovery Hook、通知字段和前端配置界面。
- [ ] 支持 CHANGELOG 自定义功能
- [ ] 更多通知渠道
